...
> 详见 [../backend/roadmap.md](../backend/roadmap.md)。
...
- `extraction_confidence`: Fixed at extraction by `source_type` provenance (immutable)
- `evidence_alpha` / `evidence_beta`: Beta posterior over fact truth (init 1.0 / 1.0)
- `fact_confidence` = `α / (α + β)`; recall uses `effective_confidence = extraction_confidence × fact_confidence`
...
Second LLM challenges extracted observations before a candidate is promoted. **Not in MVP** — MVP uses simple evidence-text substring check as a basic quality gate (design §2.4, `evidence_is_supported`).

**Placement**: adversarial validation runs at **Promote** (low frequency — a candidate reaching cluster threshold), not at Extract (high frequency — every session). This keeps the second-LLM cost off the ingest hot path.
...
Generate vector embeddings for observations and later concept candidates/concepts.
...
**Implemented default**: OpenAI-compatible `/v1/embeddings` provider, default model `BAAI/bge-m3`, default dimension `1024`. `EmbeddingConfig.api_url/api_key` override the embedding endpoint; empty values fall back to `LlmConfig` so one local server can serve both chat and embeddings.
...
**Storage**: `embedding` table stores little-endian `f32` BLOB vectors keyed by `(source_type, source_id)`. Search loads workspace rows and computes cosine similarity in Rust. sqlite-vec remains a future performance optimization, not the current default.
...
```rust
fn combined_distance(obs_a: &Observation, obs_b: &Observation, embedding_dist: f64) -> f64 {
    ...
    let intersection = entities_a.intersection(&entities_b).count() as f64;
    let union = entities_a.union(&entities_b).count() as f64;
...
**Co-occurrence signal**: observations sharing an `extraction_batch_id` (co-claimed in one `extract_observations()` call) carry an affinity edge in `observation_coclaim`. The cluster engine treats a shared batch as a positive affinity signal — see `find_coclaim()` (p2c).
...
9. Execute auto promote/demote decisions (based on vitality — see [quality-control.md](./quality-control.md))
...
Assess concept quality using the multi-dimensional vitality model. Full definition and the decay model live in [quality-control.md](./quality-control.md).

**Concept Vitality Score** (recency uses Ebbinghaus + rehearsal; `λ_eff = λ_base / (1 + 0.5·successful_recall_count)`, `λ_base = 1/90`):
```rust
fn compute_vitality(concept: &Concept) -> f64 {
    ...
    let lambda_eff = (1.0 / 90.0) / (1.0 + 0.5 * concept.successful_recall_count as f64);
...
> Decay is a **recall-time salience factor**, not an evidence mutation. There is intentionally no `update_with_decay` on `BetaConfidence`. Rationale in [quality-control.md §Time Decay](./quality-control.md#time-decay).
...
[Predictive Coding] LLM Extractor → only novel observations + confirming evidence
  ↓
[Adversarial Validator] Validator LLM challenges → accepted / adjusted / rejected   (at Promote, not Extract)
  ↓
[Hippocampal Buffer] fast_stored (24h cooling period)
  ↓
[Embedding] embed observation text → vec_embedding
  ↓
[HAC + Entity Overlap + Coclaim] cluster → concept_candidate
  ↓
...
[Chunking] hierarchy detection → parent/child relationships
  ↓
[Vitality Model] auto promote/demote   (recency = Ebbinghaus + rehearsal; decay never mutates α/β)
  ↓
